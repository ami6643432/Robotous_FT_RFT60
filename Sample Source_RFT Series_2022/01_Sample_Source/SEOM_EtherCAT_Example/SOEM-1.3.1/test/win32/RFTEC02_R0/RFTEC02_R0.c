/** \file
 * \brief Example code for RFT F/T sensor
 * EtherCAT Master: Simple Open EtherCAT master
 *
 * Usage : RFTECx_xx.exe [ifname1]
...
#define RFTEC02_SLAVE_ID 1 // make sure the id of slave index
...
void changeRFTEC02_ConfigParam( unsigned char param1, unsigned char param2, unsigned char param3, unsigned char param4 )
{
	uint8 *data_ptr;

   data_ptr = ec_slave[RFTEC02_SLAVE_ID].outputs;
...
void updateRFT_Data( void )
{
	...
	for( i = 0; i < 8; i++ )
		RFT_DATA_FIELD[i] = ec_slave[RFTEC02_SLAVE_ID].inputs[IDX_D1 + i];
	for( i = 0; i < 8; i++ )
		RFT_DATA_FIELD[i + 8] = ec_slave[RFTEC02_SLAVE_ID].inputs[IDX_D9 + i];
	
	for( i = 0; i < 6; i++ )
	{
		cvtBufferToInt16( RFT_FT_RAW + i, ec_slave[RFTEC02_SLAVE_ID].inputs + IDX_RAW_FT1 + i * 2 );
	}
	
	RFT_OverloadStatus = ec_slave[RFTEC02_SLAVE_ID].inputs[IDX_OVERLOAD];
	RFT_ErrorFlag = ec_slave[RFTEC02_SLAVE_ID].inputs[IDX_ERRLR_FLAG];	
...
//DWORD WINAPI RFT_DATA_DISPLAY_THREAD( LPVOID lpParam ) 
...
void update_RFTEC02_R0_OutputProcessData(void)
{
	...
	{
		changeRFTEC02_ConfigParam( 0, 0, 0, 0 ); 
		configParam[0] = configParam[1] = configParam[2] = 0;
	}
	else
	{
		changeRFTEC02_ConfigParam( configCmdType, configParam[0], configParam[1], configParam[2] ); 
	}	
}
...
void RFTEC02_R0_TEST(char *ifname)
{
         ...
         if (ec_slave[0].state == EC_STATE_OPERATIONAL )
         {
			...
			while(isRun)
			{
				...
				if( (cmd == 'E') || (cmd == 'e') ) // exit
				{
					...
					printf("BYE RFTEC02 SIMPLE TEST\n");
				}
...
int main(int argc, char *argv[])
{
   ...
   {      
	  ...
	  osal_thread_create(&thread2, NULL, &RFT_DATA_DISPLAY_THREAD, (void*) &ctime);
	  
      strcpy(ifbuf, argv[1]);
      /* start cyclic part */
      RFTEC02_R0_TEST(ifbuf);
   }
   else
   {
