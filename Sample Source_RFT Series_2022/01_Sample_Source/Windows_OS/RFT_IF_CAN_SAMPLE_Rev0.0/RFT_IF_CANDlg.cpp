...
#include "stdafx.h"
#include "RFT_IF_CAN.h"
#include "RFT_IF_CANDlg.h"
...
void CRFT_IF_CANDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_OPMODE, m_combo_opMode);
	DDX_Text(pDX, IDC_EDIT_RX_ID, m_nRxID_01);
	DDX_Text(pDX, IDC_EDIT_RX_ID2, m_nRxID_02);
	DDX_Text(pDX, IDC_EDIT_TX_ID, m_nTxID);
}
...
BOOL CRFT_IF_CANDlg::OnInitDialog()
{
	...
	m_combo_opMode.InsertString(0, "STANDARD");
	m_combo_opMode.InsertString(1, "EXTENDED");
	m_combo_opMode.SetCurSel(0);

	m_combo_baudrate.InsertString(0, "10Kbps");
	m_combo_baudrate.InsertString(1, "20Kbps");
	m_combo_baudrate.InsertString(2, "50Kbps");
	m_combo_baudrate.InsertString(3, "100Kbps");
	m_combo_baudrate.InsertString(4, "125Kbps");
	m_combo_baudrate.InsertString(5, "250Kbps");
	m_combo_baudrate.InsertString(6, "500Kbps");
	m_combo_baudrate.InsertString(7, "800Kbps");
	m_combo_baudrate.InsertString(8, "1000Kbps");
	m_combo_baudrate.SetCurSel(8);
...
void CRFT_IF_CANDlg::OnBnClickedCheckInterfaceOpen()
{
	...
	if (isOn)
	{
		int opMode = m_combo_opMode.GetCurSel();
		int baudrate = m_combo_baudrate.GetCurSel();
		bool enableCallback = true;

		if (m_RFT_IF.run(opMode, baudrate, enableCallback, m_fDivider_Force, m_fDivider_Torque, m_nTxID, m_nRxID_01, m_nRxID_02)) // true - enable call back...
		{
			((CButton*)GetDlgItem(IDC_CHECK_INTERFACE_OPEN))->SetWindowTextA("Close");
		...
		else
		{
			...
			((CButton*)GetDlgItem(IDC_CHECK_INTERFACE_OPEN))->SetWindowTextA("CAN Open");
...
void CRFT_IF_CANDlg::OnBnClickedCheckBias()
{
	...
	{
		m_RFT_IF.set_FT_Bias(m_nTxID, 1);
	}
	else
	{
		m_RFT_IF.set_FT_Bias(m_nTxID, 0);
	}
}
...
void CRFT_IF_CANDlg::OnBnClickedCheckFtOutCont()
{
	...
	{
		m_RFT_IF.rqst_FT_Continuous(m_nTxID);
...
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_RFT_IF.rqst_FT_OverloadCnt(m_nTxID);
...
void CRFT_IF_CANDlg::OnBnClickedButtonFilterSetting()
{
	...
	m_RFT_IF.set_FT_Filter_Type(m_nTxID, param[0], param[1]);
	int waitTimeOutCnt = 0;
