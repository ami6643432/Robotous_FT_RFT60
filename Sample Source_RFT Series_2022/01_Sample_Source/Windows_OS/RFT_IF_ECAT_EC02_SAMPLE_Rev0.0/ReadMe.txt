================================================================================
    MFC 라이브러리 : RFT_IF_ECAT 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 RFT_IF_ECAT 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

RFT_IF_ECAT 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
RFT_IF_ECAT.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CRFT_IF_ECATApp 응용 프로그램 클래스가 선언되어 있습니다.

RFT_IF_ECAT.cpp
    이는 응용 프로그램 클래스 CRFT_IF_ECATApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\RFT_IF_ECAT.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 RFT_IF_ECAT.rc에 의해 포함됩니다.
...
RFT_IF_ECATDlg.h, RFT_IF_ECATDlg.cpp - 대화 상자
    이 파일에는 CRFT_IF_ECATDlg 클래스가 포함됩니다. 이 클래스는 응용 프로그램의 주 대화 상자에 대한 동작을 정의합니다. 이 대화 상자 템플릿은 RFT_IF_ECAT.rc에 있으며, Microsoft Visual C++에서 직접 편집할 수 있습니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 RFT_IF_ECAT.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.

Resource.h
    새 리소스 ID를 정의하는 표준 헤더 파일입니다. Microsoft Visual C++에서 이 파일을 읽고 업데이트합니다.

RFT_IF_ECAT.manifest
